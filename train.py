...
# HYPERPARAMETERS
epochs = 1000 #how many policy upgrades should we do
N = 20 #how many trajectories should we collect for each V fitting
...
for i in range(epochs):
    ...
    with torch.no_grad():
        ...
        for j in range(N):
            ...
            while not episode_over:
                ...
                output = model(torch.as_tensor(observation, dtype = torch.float32)) 
        ...
        print(f"Epoch {i} reward is  {avg_reward}") 
        if (avg_reward > 450):
            print("Done")
            break
    ...
    with torch.no_grad():
        for T in lengths: #big T is length of hte peisode
            for j in range(T): 
                ...
                if (j + tobootstrap < T):
                    target+= (gamma ** tobootstrap) * V(observations[k+tobootstrap]).item()
    ...
    advantage = (advantage - advantage.mean()) / (advantage.std() + 1e-8) #was very unstable -- added this
    for j in range(M): 
        k=0
        ...
        for T in lengths:
            for j in range(T):
                ...
                k += 1
        tot = -tot
       #  print(f"Epoch {i} | Step {j} | Policy loss: {tot.item():.4f}") 
